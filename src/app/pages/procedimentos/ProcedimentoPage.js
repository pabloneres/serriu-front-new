...
import { store, index, getProcedimentos, orcamento, show, destroy, aprovar, indexOrcamentosAbertos } from '~/app/controllers/orcamentoController';
...
export function ProcedimentoPage() {
  ...
  useEffect(() => {
    indexOrcamentosAbertos(authToken)
    .then( ({data}) => {
      console.log(data)
      setOrcamentos(data)
    }).catch((err)=>{
...
function verifyAprovado(el) {
  ...
  function HandleOrcamento() {
    ...
    if (!showForm) {
      return (
        <Card>
          <CardHeader title="Orçamentos em andamento">
            <CardHeaderToolbar>
            </CardHeaderToolbar>
          </CardHeader>
          <CardBody>
