...
export function Dados(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setStatus, setSubmitting }) => {
      console.log(values)
      update(token, params.id, {...values, recebeComissao: recebeComissao === true ? 1 : 0})
  ...
  useEffect(() => {
    show(token, '/users', params.id)
      .then(({data}) => {
        setUser(data)
        setClinicasAcessos(data.acessos.map(item => Number(item.clinic_id)))
        // setDentist(data[0])
  ...
  return (
    <Card>
      <CardHeader title="Dados pessoais"></CardHeader>
      <CardBody>
        <Form
        ...
        >
          ...
          <div className="separator separator-solid mt-4 mb-4"></div>
          <h4 className="mb-4">Clinicas </h4>
          <Checkbox.Group 
            options={clinics.map(clinic => ({...clinic, label: clinic.name, value: clinic.id }))} 
            onChange={(e) => setClinicasAcessos(e)} 
            value={clinicasAcessos}
          />
