...
export function Financeiro(props) {
  ...
  const [resumoCobranca, setResumoCobranca] = useState(undefined);
  ...
  if (modalInfo) {
    show(authToken, "/dentist", modalInfo.avaliador).then(({ data }) => {
      setAvaliador(data[0]);
    });
  }

  useEffect(() => {
    index(authToken, `/pagamento?procedimento_id=${modalReceber}`).then(
      ({ data }) => {
