...
export function TabelaPreco() {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setStatus, setSubmitting, resetForm }) => {
      store(token, '/preco', {...values, clinic_id: selectedClinic.id})
        .then(() => {
          resetForm()
          setShow(false)
        })
  ...
  const formik2 = useFormik({
    ...
    onSubmit: (values, { setStatus, setSubmitting, resetForm }) => {
      update(token, '/preco', tableEdit[0],  values)
        .then(() => {
          resetForm()
          setShowEdit(false)
          setReload(!reload)
        ...
        <Table striped bordered hover>
          ...
          {tabelas.map( tabela => (
            <tr key={tabela.id} >
              ...
              {/* <span onClick={() => history.push(`/tabela-precos/${tabela.id}//precos`) }  style={{"cursor": "pointer"}} className="svg-icon menu-icon">
